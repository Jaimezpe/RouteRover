...
function createFileOnGitHub(longUrl) {
    ...
    .then(response => {
        if (response.ok) {
            ...
            shortenedUrlElement.innerHTML = '<p>New URL: <a href="' + shortUrl + '">' + shortUrl + '</a></p>';

            // Agregar el nombre del archivo creado debajo del enlace
            var fileNameElement = document.createElement("p");
            fileNameElement.textContent = "File Name: " + randomName + ".html";
            shortenedUrlElement.appendChild(fileNameElement);
        } else {
